...
router.get('/:carId', async (req, res) => {
    ...
    try {
        serviceList = await utils.getCarServices(req.user.id, req.params.carId);
...
router.post('/:carId', async (req, res) => {
    ...
    //Since serviceId is generated by the database and carId is a parameter, only mileage is the mandatory checkable field
    if ('mileAge' in req.body == false || req.body.mileAge == ""){
        ...
        try {
            ...
            console.log(error);
            res.status(400);
            res.json({message: "Error while trying to create a service!"});
        }
    }
...
router.get('/:carId/:serviceId', async (req, res) => {
    ...
    try {
        serviceList = await getService(req.user.id, req.params.carId, req.params.serviceId);
...
router.put('/:carId/:serviceId', async (req, res) => {
    ...
    let comaparative = [];
    comaparative = await utils.getService(req.user.id, req.params.carId, req.params.serviceId);
